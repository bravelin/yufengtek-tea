...
<template>
    <Plane class="tea-varieties-wrap" :full="teaVarietiesFullState">
        ...
        <div class="chart-title" v-show="varietiesDatas.length"><div>{{ varietiesTotalData }}<span>亩</span></div><h4>种植比例</h4></div>
        ...
        methods: {
            ...
            doHandlerData (datas) {
                ...
                for (let i = 0; i < datas.length; i++) {
                    ...
                    legendData.push({ name: item.varieties, ratio: ((item.area / totalData) * 100).toFixed(2) + '%', value: item.area })
                }
